...
class comicpress_control_panel_widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			array(
				'classname'   => __CLASS__,
				'description' => __( 'Login/Logoff menu with register/lost password links if not logged on. (use only if registrations are enabled).', 'comicpress' ),
			) // Args.
		);
	}
	...
	function comicpress_show_control_panel() {
		...
		if ( ! is_user_logged_in() ) {
			...
			if (is_multisite()) {
				?>
				<li>
					<a href="<?php echo site_url(); ?>/wp-signup.php"><?php _e( 'Register', 'comicpress' ); ?></a>
				</li>
				<?php
			} else {
				?>
				<li>
					<a href="<?php echo site_url(); ?>/wp-register.php"><?php _e('Register', 'comicpress' ); ?></a>
				</li>
				<?php
			}
			?>
			<li>
				<a href="<?php echo site_url(); ?>/wp-login.php?action=lostpassword"><?php _e( 'Recover password', 'comicpress' ); ?></a>
			</li>
			</ul>
			<?php
		} else {
			...
			<ul>
				<li>
					<a href="<?php echo wp_logout_url (get_permalink() ); ?>"><?php _e( 'Logout', 'comicpress' ); ?></a>
				</li>
				<?php
				wp_register();
				?>
				<li>
					<a href="<?php echo site_url(); ?>/wp-admin/profile.php"><?php _e( 'Profile', 'comicpress' ); ?></a>
				</li>
			</ul>
			<?php
		}
