...
class comicpress_control_panel_widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(
			...
			array(
				'classname'   => __CLASS__,
				'description' => __( 'Login/Logoff menu with register/lost password links if not logged on. (use only if registrations are enabled).', 'comicpress' ), // Args.
			)
		);
	}
	...
	function comicpress_show_control_panel() {
		...
		if ( ! is_user_logged_in() ) {
			...
			if ( is_multisite() ) {
				...
				<li>
					<a href="<?php echo site_url(); ?>/wp-register.php">
