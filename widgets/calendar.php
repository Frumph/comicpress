...
function comicpress_get_calendar( $initial = true, $echo = true, $taxonomy = 'post' ) {
	...
	// Weeek_begins = 0 stands for Sunday.
	...
	else return apply_filters( 'get_comicpress_calendar', $calendar_output );
...
class comicpress_calendar_widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			array(
				'classname'   => __CLASS__,
				'description' => __( 'Display a calendar showing this months posts. (this calendar does not drop lines if there is no title given.)', 'comicpress' ), // Args.
			)
		);
	}
						...
						<div class="wp-calendar-download-links">
							...
							if ( ! empty( $small ) || ! empty( $medium ) || ! empty( $large ) ) {
								...
								foreach ( array(
									...
									<a href="<?php echo esc_attr( ${$field} ); ?>" title="<?php echo esc_attr( $text[0] ); ?>">
	...
	function form( $instance ) {
		...
		foreach ( array(
			...
			'link'      => array(
				'label' => __( 'Add link on thumbnails:', 'comicpress' ),
				'after' => '<hr />',
			),
