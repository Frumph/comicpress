...
if (!function_exists('comicpress_display_social_icons')) {
	function comicpress_display_social_icons() {
		...
		if (!empty($googleplus)) $output .= '<a href="'.$googleplus.'" target="_blank" title="'.__('Check me out on Google+','comicpress').'" class="menunav-social menunav-googleplus">'.__('Google+','comicpress').'</a>'."\r\n";
		...
		if (!empty($youtube)) $output .= '<a href="'.$youtube.'" target="_blank" title="'.__('View my YouTube','comicpress').'" class="menunav-social menunav-youtube">'.__('YouTube','comicpress').'</a>'."\r\n";
 ...
 */
function comicpress_register_required_plugins() {
    ...
    $plugins = array(
        ...
        array(
            'name'      => 'ComicPress to Comic Easel',
            'slug'      => 'cp2ce',
            'required'  => false,
        ),
		array(
...
if (is_admin()) {
	@require_once(comicpress_themeinfo('themepath').'/lib/class-tgm-plugin-activation.php');
